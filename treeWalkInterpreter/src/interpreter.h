...
Literal visitBinary(Expr* expr) {
    ...
    if (type == NUMBER_LITERAL) return number;
    if (type == BOOL_LITERAL) return boolean;
...
Literal isTruthy(Literal value) {
    ...
    if (value.type == BOOL_LITERAL) return value;
...
Literal isEqual(Literal left, Literal right) {
    ...
    if (left.type == NUMBER_LITERAL && right.type == NUMBER_LITERAL) {
        return (Literal){
            .type = BOOL_LITERAL,
            .object.boolean = left.object.number == right.object.number};
    }
